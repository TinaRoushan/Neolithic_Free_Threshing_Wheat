library(Momocs)
library(geomorph)
#Methodology: Repeatability and measurement error
...
NeoRefOut3<-coo_center(NeoRefOut2)
#lateral
NeoRefOut.l <- filter(NeoRefOut2, View == "l")#creates subset of lateral views only
calibrate_harmonicpower_efourier(NeoRefOut.l,nb.h=8)#tells you how many harmonics will be needed to gather x% of harmonic power- here 8= 99%
...
rgl.snapshot("modernaccessions2.png","png")


#Same as above but landraces only
...
NeoRefOut3_LR<-coo_center(NeoRefOut2_LR)
#lateral
NeoRefOut_LR.l <- filter(NeoRefOut3_LR, View == "l")#creates subset of lateral views only
...
#dorsal
NeoRefOut_LR.d <- filter(NeoRefOut3_LR, View == "d")#creates subset of lateral views only
calibrate_harmonicpower_efourier(NeoRefOut_LR.l,nb.h=12)#tells you how many harmonics will be needed to gather x% of harmonic power- here 8= 99%
...
#Stage 2a comparison with archaeobotanical material (all material)
NeoCombCoords <- list.files("D:\\Coordinates\\Free Threshing\\Combined_Neo", full.names = TRUE)
NeoCombFrame <- read.csv("D:\\Coordinate_Frames\\Combined_Neo.csv", header = TRUE)
...
NeoCombOut3<-coo_center(NeoCombOut2)
#lateral
NeoCombOut.l <- filter(NeoCombOut3, View == "l")#creates subset of lateral views only
...
#dorsal
NeoCombOut.d <- filter(NeoCombOut3, View == "d")#creates subset of lateral views only
...
###
#Stage 2a comparison with archaeobotanical material - Land races
#Catal
NeoCombCoords_LRCatal <- list.files("D:\\Coordinates\\Free Threshing\\Combined_Neo_LR_Catal", full.names = TRUE)
NeoCombFrame_LRCatal <- read.csv("D:\\Coordinate_Frames\\Combined_Neo_LR_Catal.csv", header = TRUE)
...
NeoCombOut3_LRCatal <-coo_center(NeoCombOut2_LRCatal)
#lateral
...
NeoCombFrame_LRKouph <- read.csv("D:\\Coordinate_Frames\\Combined_Neo_LR_Kouph.csv", header = TRUE)
...
#Reclassification of archaeological against modern landraces 
##unclass Catal
...
UnclassOut2 <-coo_center(UnclassOut1)
#reclass
...
#reclassify against modern landrace/trad varieties
reLDA(UnclassOut.d.l.efour, NeoRefOut_LR.d.l.lda)
#Same as above but with priors removed (not biased by group size)
...
UnclassOut2K <-coo_center(UnclassOut1K)
#reclass
UnclassOut.dK <- filter(UnclassOut2K, View == "d")
UnclassOut.lK <- filter(UnclassOut2K, View == "l")
...
NeoRefFrame_PW <- read.csv("D:\\Coordinate_Frames\\Combined_Neo_LR.csv", header = TRUE)
...
#Stage 2b: Varietal comparisons 

#Catal visual comparison with hexaploid accessions 
...
#LDA of all 8 hexaploid accessions
NeoRefCoords_var <- list.files("D:\\Coordinates\\Free Threshing\\Neo_Hex", full.names = TRUE)
NeoRefFrame_var <- read.csv("D:\\Coordinate_Frames\\Neo_Hex.csv", header = TRUE)
NeoRefTxt_var <- import_txt(NeoRefCoords_var, fileEncoding="UTF-8-BOM")#fileencoding gets rid of weird symbols
NeoRefOut_var <- Out(NeoRefTxt_var, fac=NeoRefFrame_var) #creates an Out object from a specified list- you can specify landmarks in "ldk"
NeoRefOut2_var <-coo_scale (NeoRefOut_var)
NeoRefOut3_var <-coo_scale (NeoRefOut2_var)
#lateral
NeoRefOut.l_var <- filter(NeoRefOut2_var, View == "l")#creates subset of lateral views only
NeoRefOut.l.efour_var <- efourier(NeoRefOut.l_var, nb.h=8, norm = FALSE, start = TRUE)# for why norm= false see note on ?efourier regarding roughly circular objects
#dorsal
NeoRefOut.d_var <- filter(NeoRefOut2_var, View == "d")#creates subset of lateral views only
NeoRefOut.d.efour_var <- efourier(NeoRefOut.d_var, nb.h=8, norm = FALSE, start = TRUE)# for why norm= false see note on ?efourier regarding roughly circular objects
#combining just dorsal and lateral
NeoRefOut.d.l_var <- combine (NeoRefOut.d_var, NeoRefOut.l_var)#dataset of just dorsal and lateral views
#?LDA as classified by taxa
NeoRefOut.d.l_var %>% chop(~View) %>% lapply(efourier,nb.h=8, norm = FALSE, start = FALSE) %>% combine %>% LDA ('taxon.code', scale=FALSE, center= TRUE) 
NeoRefOut.d.l.lda_var <- NeoRefOut.d.l_var %>% chop(~View) %>% lapply(efourier,nb.h=8, norm = FALSE, start = FALSE, center= TRUE) %>% combine %>% LDA ('taxon.code', scale=FALSE, center= TRUE)
#LDA as classified by accession
NeoRefOut.d.l_var %>% chop(~View) %>% lapply(efourier,nb.h=8, norm = FALSE, start = FALSE) %>% combine %>% LDA ('Accessioncode', scale=FALSE, center= TRUE) 
NeoRefOut.d.l.lda_var_C <- NeoRefOut.d.l_var %>% chop(~View) %>% lapply(efourier,nb.h=8, norm = FALSE, start = FALSE, center= TRUE) %>% combine %>% LDA ('Accessioncode', scale=FALSE, center= TRUE)
#reclass
reLDA(UnclassOut.d.l.efour, NeoRefOut.d.l.lda_var_C)


#Kouphovouno visual comparison with tetraploid accessions 
NeoRefCoords_var_K <- list.files("D:\\Coordinates\\Free Threshing\\Combined_Neo_Tet", full.names = TRUE)
NeoRefFrame_var_K <- read.csv("D:\\Coordinate_Frames\\Combined_Neo_Tet.csv", header = TRUE)
...
NeoRefCoords_var_K <- list.files("D:\\Coordinates\\Free Threshing\\Neo_Tdur", full.names = TRUE)
NeoRefFrame_var_K <- read.csv("D:\\Coordinate_Frames\\Neo_Tdur.csv", header = TRUE)
NeoRefTxt_var_K <- import_txt(NeoRefCoords_var_K, fileEncoding="UTF-8-BOM")#fileencoding gets rid of weird symbols
NeoRefOut_var_K <- Out(NeoRefTxt_var_K, fac=NeoRefFrame_var_K) #creates an Out object from a specified list- you can specify landmarks in "ldk"
NeoRefOut2_var_K <-coo_scale (NeoRefOut_var_K)
#lateral
...
#Procrustes ANOVA of PCA scores for Kouph group vs Catal 
library(Momocs)
library(geomorph)
#Catal Coords
UnclassCoords <- list.files("D:\\Coordinates\\Free Threshing\\Unclass_Catal", full.names = TRUE)
...
UnclassOut.l.efour <- efourier(UnclassOut.l, nb.h=8, norm = FALSE, start = FALSE)
UnclassOut.d.efour <- efourier(UnclassOut.d, nb.h=8, norm = FALSE, start = FALSE)
#Kouph Coords
UnclassCoordsK <- list.files("D:\\Coordinates\\Free Threshing\\Unclass_Kouph", full.names = TRUE)
UnclassFrameK <- read.csv("D:\\Coordinate_Frames\\Unclass_Kouph.csv", header = TRUE)
UnclassTxtK <- import_txt(UnclassCoordsK, fileEncoding="UTF-8-BOM")#fileencoding gets rid of weird symbols
...
UnclassOut.lk.efour <- efourier(UnclassOut.lK, nb.h=8, norm = FALSE, start = FALSE)
UnclassOut.dk.efour <- efourier(UnclassOut.dK, nb.h=8, norm = FALSE, start = FALSE)
#export
export(UnclassOut.l.efour )
export(UnclassOut.d.efour)
export(UnclassOut.lk.efour)
export(UnclassOut.dk.efour)
Catal_l <- read.csv("D:\\Coordinate_Frames\\Catal_l.csv")
Catal_d <- read.csv("D:\\Coordinate_Frames\\Catal_d.csv")
Kouph_l <- read.csv("D:\\Coordinate_Frames\\Kouph_l.csv")
Kouph_d <- read.csv("D:\\Coordinate_Frames\\Kouph_d.csv")
KouphCatal_d<- read.csv("D:\\Coordinate_Frames\\KouphCatal_d.csv")
KouphCatal_l<- read.csv("D:\\Coordinate_Frames\\KouphCatal_l.csv")
#Calculate PCA scores for above
PCAObservation_d<-prcomp(KouphCatal_d )$x 
PCAObservation_l<-prcomp(KouphCatal_l)$x 
#Geomorph data frames for PCA computations of coefficients
gdf_d<-geomorph.data.frame(shape=PCAObservation_d) #ID: a vector containing the specimens ID 
gdf_l<-geomorph.data.frame(shape=PCAObservation_l) #ID: a vector containing the specimens ID 
#Creates 5 level session factor (consecutive)
randomisedfactor<-as.factor(rep(1:5, 32))
randomisedfactor
sitefactor<-as.factor(gl(2, 80))
sitefactor
#Procrustes ANOVA using randomised factor and then site factor as dependent variables
#Random factor= not significant
summary(procD.lm(shape~randomisedfactor, data=gdf_d))
summary(procD.lm(shape~randomisedfactor, data=gdf_l))
#Site factor= significant
summary(procD.lm(shape~sitefactor, data=gdf_d))
summary(procD.lm(shape~sitefactor, data=gdf_l))
